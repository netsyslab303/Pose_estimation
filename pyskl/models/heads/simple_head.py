...
class SimpleHead(BaseHead):
    ...
    def __init__(self,
        ...
        #self.fc = nn.Conv2d(self.in_c, self.num_classes, kernel_size=1)
    ...
    def predict(self, x):
        ...
        if self.mode == 'GCN':
            _, m, c, t, v = x.size()
            feature = x.view(1, m, c, t, v).permute(0, 2, 3, 4, 1)

            # prediction
            # for i in range(m):
            #     tmp = self.fc_cls(x[0][i])
            #
            # output = tmp.view(1, m, -1, t, v).permute(0, 2, 3, 4, 1)
