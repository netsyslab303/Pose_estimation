...
def inference_recognizer_person(model, video, outputs=None, as_tensor=True, **kwargs):
    """Inference a video with the recognizer.
    ...
    test_pipeline = Compose(test_pipeline)
    test_pipeline.person = True
    data = test_pipeline(data)
    data_numpy = data['keypoint'].numpy()
    data = collate([data], samples_per_gpu=1)
    #N, C, T, V, M = data['keypoint'].size()
